<x-app-layout>
        ...
        <table id="milktea-relationships-table" class="min-w-full bg-white border border-gray-300 rounded-lg">
            ...
            <tbody id="milktea-relationships-body">
                ...
                <tr>
                    ...
                    <td class="py-2 px-4">
                        <form id="delete-form-{{ $relationship->id }}" action="{{ route('milkteasize.destroy', $relationship->id) }}" method="POST" class="inline-block ml-2">
                            @csrf
                            @method('DELETE')
                            <button type="button" class="bg-red-500 text-white py-2 px-4 rounded hover:bg-red-700" onclick="confirmDelete('{{ $relationship->id }}')">Delete</button>
                        </form>
                    </td>
    ...
    <script>
        function confirmDelete(relationshipId) {
            Swal.fire({
                title: 'Are you sure?',
                ...
                if (result.isConfirmed) {
                    document.getElementById('delete-form-' + relationshipId).submit();
                }
            });
        }
    </script>
